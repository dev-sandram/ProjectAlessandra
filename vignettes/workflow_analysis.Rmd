...
8.  Output --- Returns mean_counts, a table with mean counts for the most variable genes along with cohort and condition annotations.

### Syntax example for bulk_counts_summary_dt function

```{r}
mean_counts <- combine_cohorts_dt(
...
```{r, echo=FALSE, results='asis'}
knitr::kable(head(mean_counts, 10), caption = "Top 100 most variable genes with cohort and condition annotations")
```
...
-   Percentages within cluster and within cell type;
...
Beyond base R and data.table, several other packages can perform the same tasks. The tidyverse—especially dplyr—offers very readable syntax (Wickham et al., 2023), though it is generally slower on large joins and aggregations. dtplyr provides a middle ground by translating dplyr code into data.table for better performance (Hinds & Wickham, CRAN). For very large datasets or out-of-memory processing, tools such as Apache Arrow (Arrow Project, 2023) and DuckDB (Raasveldt & Mühleisen, 2019) are widely recognized as efficient alternatives. 
